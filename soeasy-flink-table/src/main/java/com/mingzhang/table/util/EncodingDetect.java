...
public class EncodingDetect {
    ...
    public static void main(String[] args) throws Exception {
        String filePath = "C:\\Users\\lenovo\\Desktop\\ALL";
        String disFile = "C:\\Users\\lenovo\\Desktop\\ALL\\TRANSCODE\\newfile.csv";

        File fileDir = new File(filePath);
        File[] files = fileDir.listFiles();
        for(File file : files){
            String encode=getJavaEncode(file.getPath());
...
//		readFile(file,encode);
     ...
     * @return 文件的编码
     */
    public static String getJavaEncode(String filePath) throws Exception{
...
class BytesEncodingDetect extends Encoding {
     ...
     */
    public int detectEncoding(File testfile) throws Exception{
        FileInputStream chinesefile;
        byte[] rawtext;
//	    rawtext = new byte[(int) testfile.length()];
        long fileLength = testfile.length();
        Integer length = 61858764;
        if(fileLength > 61858764 || fileLength < 0){
            length = 61858764;
        }else{
        ...
        try {
            ...
            throw new Exception("获取编码类型失败：", e);
        }
